...
properties_link=https://raw.githubusercontent.com/Reorks9/Dnipro_DevOps_part2/master/project/config/testmodel.properties
# database-1.c4oxcsltuma8.eu-central-1.rds.amazonaws.com
...
if test ! -f $PROP_FILE; then
    ...
    cd $IMDIR
    wget $properties_link -O $PROP_FILE
    ...
    sed -i=bak -e "s/localhost/$SERVER_DNS/g" $PROP_FILE
fi

echo "------> Checking databases..."
if test ! -f ~/.pgpass; then
    echo "$SERVER:5432:$PSQL_USER:$PSQL_USER:$PSQL_PWD" > ~/.pgpass;
    chmod 0600 ~/.pgpass;
fi;
for db in $USERPROFILEDB $PRODDB; do
    psqldbs=$(psql -h $SERVER -d $PSQL_USER --list);
    if echo $psqldbs | egrep -q '\s'$db'\s'; then
        echo $db exists.
    else
        echo Creating $db
        psql -h $SERVER -d $PSQL_USER -c "CREATE DATABASE \"$db\";";
    fi
done
